...
-- Articles table (matching your local schema)
...
CREATE TABLE queries (
  ...
  response TEXT,
  chunks_used INTEGER[],
  feedback SMALLINT, -- 1 for 👍, -1 for 👎, NULL for no feedback
...
CREATE POLICY "Chunks are publicly readable" ON chunks FOR SELECT USING (true);
CREATE POLICY "Embeddings are publicly readable" ON embeddings FOR SELECT USING (true);

-- Create policy for queries (insert only for analytics)
CREATE POLICY "Queries can be inserted by anyone" ON queries FOR INSERT WITH CHECK (true);